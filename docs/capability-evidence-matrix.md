...
| Actionable observability | versioned API alerts plus atomic textfile metrics for scheduled backup/integrity operations, preserving last-success time on failure | unit/CLI success-failure tests and synthetic `promtool` firing drills for API, operation failure and stale backup; collector mounting, receiver credentials and delivery remain deployment-specific |
...
| Release-bound environment evidence | schema-v2 fail-closed manifest binds an independent verifier, UTC freshness, durable evidence URI and SHA-256 evidence digest to an exact 40-character release SHA; age override is capped at seven days | positive and self-approval/missing-digest/pending/wrong-release negative tests plus `admission-verify` non-zero CLI contract |
