...
| Actionable observability | versioned API alerts plus atomic textfile metrics for scheduled backup/integrity/database-health operations, preserving last-success time on failure; database exporter emits low-cardinality connection utilization | unit/CLI tests and synthetic `promtool` firing drills for API, operation/exporter failure, database saturation and stale backup; monitoring identity, collector mounting, receiver credentials and delivery remain deployment-specific |
| Container supply-chain evidence | SPDX JSON SBOM, Python dependency audit and fixable High/Critical image vulnerability gate | commit-pinned SBOM/scan actions, `pip-audit`, retained CI artifact and an expiring, source-checked exception for CPython `CVE-2026-15308` because the affected HTML parser is outside the service path |
...
| Release-bound environment evidence | schema-v3 fail-closed bundle binds an independent verifier, UTC freshness, durable URI and recomputed local evidence-file SHA-256 to an exact release SHA; age override is capped at seven days | positive and tamper/path-traversal/file-reuse/self-approval/age-bypass negative tests plus `admission-verify` non-zero CLI contract |
| Repository security governance | protected `main`, strict required CI/CodeQL checks, enforced administrators, linear history, resolved discussions, no force-push/delete, secret scanning/push protection and Dependabot security updates | live GitHub protection/security API audit; settings remain externally mutable and must be rechecked at release |
