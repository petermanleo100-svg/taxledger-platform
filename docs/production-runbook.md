...
Use `taxledger-operations` for controlled backup, restore and audit verification. Set `TAXLEDGER_TEXTFILE_DIR` to a Node Exporter textfile-collector directory (or equivalent). Each command atomically writes its own bounded-label `.prom` file and preserves the last-success timestamp after a failed attempt. Schedule `backup-create`, `audit-verify`, and `database-status-export`; run the database exporter at least every five minutes using a separate read-only monitoring identity with access only to `pg_stat_activity` and `pg_settings`. It emits current connections, configured maximum and the utilization ratio without database/user labels. Alert on non-zero exit, `operation_success == 0`, `node_textfile_scrape_error{job="taxledger-node-exporter"} == 1`, database exporter age beyond ten minutes, connection utilization above 80%, or backup age beyond the 15-minute RPO. An unwritable or unreadable metrics directory is a deployment failure. The API caps bodies at 2 MiB and ledger batches at 1000 rows; tune ingress limits to the same or lower value.

Load `deploy/prometheus/taxledger-alerts.yml` into the approved Prometheus-compatible backend and map `owner=platform-operations` and `owner=database-operations` plus severity to named receivers. CI validates syntax and executes synthetic firing scenarios for readiness, 5xx rate, p95 latency, scheduled-operation failure, stale backup, database saturation and database-exporter staleness with `promtool`; receiver credentials and a real test notification remain environment acceptance evidence. CI runs `pip-audit`, retains an SPDX JSON image SBOM for 30 days, and blocks image vulnerabilities that are High/Critical with a known fix. Unfixed findings require explicit release risk review rather than being described as remediated.
